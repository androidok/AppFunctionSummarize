		...
		if(data) {
			...
			if(data1 && data1.length> 0){
				for(var j = 0 ; j<data1.length;j++){
					...
					str+='<a href="http://edu.51cto.com/course/course_id-'+data1[j].course_id+'.html" target="_blank"><img src="'+data1[j].imgUrl+'" height="63" width="86"></a>';
					str+='<dl>';
			...
			if(data2 && data2.length> 0){
				for(var i = 0; i< data2.length;i++){
					str+='<div class="vlist video-icon" style="margin: 2px 8px;">';
					str+='<a href="http://edu.51cto.com/course/course_id-'+data2[i].course_id+'.html?edu_recommend_adid=91" target="_blank"><img src="'+data2[i].imgUrl+'" height="63" width="86"></a>';
					str+='<dl>';
